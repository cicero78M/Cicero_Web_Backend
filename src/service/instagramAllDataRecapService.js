...
export async function generateInstagramAllDataRecap({
  clientId,
  roleFlag,
  clientName,
  regionalId,
  ...
  for (let i = 0; i < months.length; i += 1) {
    ...
    const { rows = [] } = await getRekapLikesByClient(
      clientId,
      'bulanan',
      monthKey,
      null,
  ...
  if (!hasData) {
    throw new Error('Tidak ada data likes Instagram untuk rentang bulan yang dipilih.');
  }

  const grandTotal = monthlyTotals.reduce((sum, value) => sum + value, 0);
  const header = ['Polres', ...months.map((m) => m.label), 'Total'];
  const aoa = [
    [`Rekap Instagram All Data – ${clientName || clientId}`],
    [
...
export default generateInstagramAllDataRecap;
