...
export async function fetchAndStoreSingleTiktokPost(clientId, videoInput) {
  ...
  return {
    ...
    systemCreatedAt: postPayload.created_at,
    likeCount,
    commentCount,
  };
}
...
async function getVideoIdsToday(clientId = null) {
  ...
  let sql =
    "SELECT video_id FROM tiktok_post WHERE DATE((COALESCE(original_created_at, created_at) AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Jakarta') = $1";
...
async function deleteVideoIds(videoIdsToDelete, clientId = null) {
  ...
  let sql =
    "DELETE FROM tiktok_post WHERE video_id = ANY($1) AND DATE((COALESCE(original_created_at, created_at) AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Jakarta') = $2";
...
export async function fetchAndStoreTiktokContent(
  ...
  let kontenHariIniSql =
    "SELECT video_id, client_id, created_at FROM tiktok_post WHERE DATE((COALESCE(original_created_at, created_at) AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Jakarta') = $1";
