...
test.each([
  ...
  const result = await authorizeReportRequest(
    request(query, { client_id: 'A', client_ids: ['A'], role: 'operator' }, headers),
  );
  expect(result.error).toEqual({ status: 403, message: 'client_id tidak diizinkan' });
});

test('multi-client token rejects a client outside its rehydrated assignments', async () => {
  const result = await authorizeReportRequest(
    request({ client_id: 'C', role: 'C' }, { client_ids: ['A', 'B'], role: 'operator' }),
  );
  expect(result.error?.status).toBe(403);
});

test('direktorat scope rejects an unverified child relationship', async () => {
  mockFindById.mockResolvedValue({ client_id: 'DITBINMAS', client_type: 'direktorat' });
  const result = await authorizeReportRequest(
    request(
